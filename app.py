...
def parse_questions_flexible(txt_content):
    ...
    for line in lines:
        ...
        question_match = re.match(r'^(?P<question_id>\d+)[\.\)\s]*\s*(?P<question_text>.+)$', line)
        if question_match:
    ...
    try:
        ...
        if questions:
            ...
            for question in questions:
                ...
                # Add type only once
                if question_type in type_tracker:
                    question_type = ''  # Clear repeated types
                else:
                    type_tracker.add(question['Type'])
        ...
        if st.button("Generate Questions"):
            if context:
                ...
                prompt = (
                    ...
                    "Specify the Bloom's taxanomy level applied at the end of each question."
                )
                ...
                if question_type == "Multiple-Choice":
                    ...
                    prompt +=  (
                        "For true/false statement, provide statement that can be answered with True or False. Title the header as $$True/False Questions$$ "
                ...
                prompt += (
                    ...
                    "$$Questions$$ "
                    "1. Question1 ..... (Bloom's taxanomy level), 2. Question2 ..... (Bloom's taxanomy level)"
